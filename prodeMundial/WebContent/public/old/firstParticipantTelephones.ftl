...
<#if ( (firstParticipant.telephoneNumbers?size)?? && firstParticipant.telephoneNumbers?size > 0 ) >
		<#list 0..firstParticipant.telephoneNumbers?size -1 as i>

			<div id="telephoneSection_${i}">
			<table border="0" cellspacing="0" cellpadding="0" >
				<tr>
					<td class="txTahomaGrs10" valign="top"><@show.invtrLabel key="firstParticipant.Label.Type"/><br>
						<select name="telephoneType_${i}" id="telephoneType_${i}" class="selectTelType">
							<option value="-1"><@show.invtrLabel key="firstParticipant.Menu.SelectType"/></option>
							<#list telephoneTypes as telephoneType>
								<option value="${telephoneType.id?c}" <#if telephoneType.id?c == firstParticipant.telephoneNumbers[i].telephoneType.id?c>selected</#if>>${telephoneType.description}</option>				
							</#list>
						</select>
						<@show.invtrErrorField key="telephoneType_${i}"/>
					</td>
					<td><img src="img/punto.gif" width="6" height="1" alt="" border="0"></td>
					<td class="txTahomaGrs10" valign="top"><@show.invtrLabel key="firstParticipant.Label.CountryCode"/><br>
						<select name="countryCode_${i}" id="countryCode_${i}" class="selectCountryCode">
							<option value="-1"><@show.invtrLabel key="firstParticipant.Menu.SelectCountryCode"/></option>
							<#list countryCodes as countryCode>
								<#if countryCode.callingCode == "" >
									<optgroup label='${countryCode.description}'></optgroup>";
								<#else>
									<option value="${countryCode.id?c}" <#if countryCode.id?c == firstParticipant.telephoneNumbers[i].countryCode.id?c>selected</#if>>${countryCode.description}</option>				
								</#if>
							</#list>
						</select>
					...
					<td><img src="img/punto.gif" width="6" height="1" alt="" border="0"></td>
					<td class="txTahomaGrs10" align="center" valign="top" nowrap ><@show.invtrLabel key="firstParticipant.Label.AreaCode"/><br>
						<@show.inputText key="areaCode_${i}" disabled="false" required="true" style="null" value="${firstParticipant.telephoneNumbers[i].areaCode}" size="10" class="inputTextCode" maxlength="4"/>
					</td>
					<td><img src="img/punto.gif" width="6" height="1" alt="" border="0"></td>
					<td class="txTahomaGrs10" valign="top"><@show.invtrLabel key="firstParticipant.Label.LocalNumber"/>
						<@show.inputText key="localNumber_${i}" disabled="false" required="true" style="null" value="${firstParticipant.telephoneNumbers[i].localNumber}" size="10" class="inputTextNum" maxlength="8"/>
...
<#else>
	...
	<table border="0" cellspacing="0" cellping="0">
		<tr>
			<td class="txTahomaGrs10" valign="top"><@show.invtrLabel key="firstParticipant.Label.Type"/><br>
